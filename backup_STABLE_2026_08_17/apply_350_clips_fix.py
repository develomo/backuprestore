...
print("🚀 Starting High-Capacity (150-350 Clips) Engine Setup...")

# 1. Backups with V2 Tag
for f in ["master_pipeline.py", "app.py"]:
  if os.path.exists(f):
    shutil.copy(f, f"{f}.bak_v2")
    print(f"📦 Backup Created: {f}.bak_v2")

# 2. Complete High-Capacity Master Pipeline Code
...
def generate_subtitle_segments(audio_path, caption_type="phrase"):
    """Generates precise timestamped subtitle segments based on exact UI mode."""
    segments = []
    cap_type = str(caption_type).lower().replace(" ", "_")
    
    try:
        import whisper
        print(f"   ➔ Transcribing voice audio with Whisper [UI Mode: {caption_type}]...")
        model = whisper.load_model("base")
        result = model.transcribe(audio_path, word_timestamps=True)

        if "word" in cap_type: # Word-by-Word Mode
        ...
        elif "line" in cap_type: # Line Mode (Groups of 5 words)
            for seg in result.get("segments", []):
                words = seg.get("words", [])
                if not words:
                    txt = seg["text"].strip().upper()
                    if txt:
        ...
        print(f"   [CAPTION ENGINE]: Generated {len(segments)} timed caption segments.")
    except Exception as e:
        print(f"   [WARN] Whisper transcription fallback: {e}")
        audio_clip = AudioFileClip(audio_path)
        dur = audio_clip.duration
        audio_clip.close()
        words = ["AUTOMATED VIDEO SYSTEM", "CREATORFLOW ENGINE V2", "AI CONTENT GENERATOR"]
        step = dur / len(words)
...
def draw_subtitles_on_frame(frame, current_time, subtitle_segments, video_size, style_config):
    """Draws Subtitles using UI Style, Colors, and Auto Word-Wrapping."""
    active_text = ""
    ...
    # Direct Style UI Parameters
    font_size_ratio = float(style_config.get("font_size_ratio", 0.038))
    font_size = max(22, int(height * font_size_ratio))
    text_color = style_config.get("text_color", (255, 235, 0))
    stroke_color = style_config.get("stroke_color", (0, 0, 0))
    bg_color = style_config.get("bg_color", (0, 0, 0, 190))
    ...
    # Wrap text to prevent going outside boundaries
    max_chars = max(10, int(width / (font_size * 0.55)))
    wrapped_lines = textwrap.wrap(active_text, width=max_chars)
...
def apply_green_screen_mask(cta_clip):
    """Removes green screen background completely."""
    try:
        def make_mask(frame):
            r = frame[:, :, 0].astype(float)
            g = frame[:, :, 1].astype(float)
            b = frame[:, :, 2].astype(float)
            is_green = (g > 75) & (g > r * 1.15) & (g > b * 1.15)
...
def safe_load_clip(clip_path, duration):
    """Safely loads video clip and verifies frame readability."""
    if not os.path.exists(clip_path) or os.path.getsize(clip_path) < 1024:
        print(f"   [SKIP EMPTY/MISSING FILE]: {os.path.basename(clip_path)}")
        return None
    try:
        vc = VideoFileClip(clip_path, audio=False)
        _ = vc.get_frame(0) # Verify first frame readable
        ...
        print(f"   [SKIP CORRUPT CLIP]: {os.path.basename(clip_path)} (Error: {err})")
        return None

def process_multi_clip_render(clips_list, audio_path, output_path, settings=None):
    print("\\n======================================================================")
    print(f"MASTER CREATORFLOW BATCH RENDER - HIGH CAPACITY ENGINE ({len(clips_list)} CLIPS)")
    print("======================================================================\\n")
    ...
    temp_chunks_dir = os.path.join(out_dir or ".", "temp_batch_chunks")
    ...
    # 3. HIGH CAPACITY BATCH-CHUNKING (HANDLES UP TO 350+ CLIPS SAFELY)
    print(f"\\n[STEP 3/5] PROCESSING {len(clips_list)} CLIPS IN 25-CLIP CHUNKS...")
    ...
    chunk_size = 25 # Safe limit per memory batch
    batch_chunk_files = []
    cut_timestamps = [0.0]
    current_time = 0.0

    for batch_idx in range(0, num_clips, chunk_size):
        batch_paths = valid_clips[batch_idx : batch_idx + chunk_size]
        loaded_batch_clips = []
        
        print(f"   ➔ Rendering Batch #{batch_idx // chunk_size + 1} ({len(batch_paths)} clips)...")
        for idx, cp in enumerate(batch_paths):
            vc = safe_load_clip(cp, clip_dur)
            if vc is not None:
                loaded_batch_clips.append(vc)
                current_time += clip_dur
                if (batch_idx + idx) < num_clips - 1:
                    cut_timestamps.append(round(current_time, 2))

        if loaded_batch_clips:
            batch_concat = concatenate_videoclips(loaded_batch_clips, method="compose")
            chunk_file = os.path.join(temp_chunks_dir, f"chunk_{batch_idx}.mp4")
            ...
            batch_chunk_files.append(chunk_file)
            
            # Explicitly close clips in this batch to release RAM & processes
            batch_concat.close()
            for c in loaded_batch_clips:
                c.close()

    if not batch_chunk_files:
        raise RuntimeError("All video clips were corrupt or unreadable.")

    # Combine pre-rendered chunks
    chunk_clips = [VideoFileClip(cf, audio=False) for cf in batch_chunk_files]
    combined_video = concatenate_videoclips(chunk_clips, method="compose")
    ...
    # 5. OVERLAYS (WATERMARK LOGO & SUBSCRIBE OVERLAY)
    ...
    if cta_path and os.path.exists(str(cta_path)):
        try:
            ...
            cta_raw = VideoFileClip(str(cta_path), audio=False).resize(width=360)
            ...
            print(f"   [APPLIED]: Subscribe Overlay with Green Screen Removal ({os.path.basename(str(cta_path))})")
    ...
    # 6. CAPTION CONFIG PARSING & CHECKBOX LOGIC
    enable_captions = settings.get("enable_captions", True)
    
    if enable_captions:
        ui_cap_cfg = settings.get("caption_config") or {}
        
        caption_type = ui_cap_cfg.get("caption_type", "phrase")
        text_color_hex = ui_cap_cfg.get("text_color", "#FFE100")
        bg_color_hex = ui_cap_cfg.get("bg_color", "#000000")
        style_id = ui_cap_cfg.get("style_id", "boxed")

        chosen_style = {
            "text_color": hex_to_rgb(text_color_hex, (255, 225, 0)),
            "stroke_color": (0, 0, 0),
            "bg_color": (*hex_to_rgb(bg_color_hex, (0, 0, 0)), 190),
            "use_box": True if "box" in str(style_id).lower() or ui_cap_cfg.get("use_box", True) else False,
            "font_size_ratio": float(ui_cap_cfg.get("font_size_ratio", 0.038))
        }

        print(f"   [BURNING CAPTIONS]: Mode = '{caption_type.upper()}', Style = '{style_id}', Text Color = {chosen_style['text_color']}")
        ...
        print("   [CAPTIONS SKIPPED]: 'Add Captions' Checkbox was unchecked in UI.")
    ...
    print("======================================================================")
    print(f"[SUCCESS] RENDER COMPLETED SUCCESSFULLY WITH {len(clips_list)} CLIPS!")
...
print(
    "✅ Updated master_pipeline.py with High-Capacity Batch-Chunking Engine!"
)

# 3. Update app.py UI Connection
with open("app.py", "r", encoding="utf-8") as f:
  app_content = f.read()

patch_code = """
        # Pass exact UI Subtitle configurations & Checkbox values to Backend
        caption_config = {
            "caption_type": st.session_state.get("caption_category_select", "phrase"),
            "text_color": st.session_state.get("caption_text_color", "#FFE100"),
            "bg_color": st.session_state.get("caption_bg_color", "#000000"),
            "style_id": st.session_state.get("caption_style_select", "boxed"),
            "use_box": True
        }
        merged_settings["caption_config"] = caption_config
        merged_settings["enable_captions"] = st.session_state.get("long_enable_captions", True) if "long" in str(st.session_state).lower() else st.session_state.get("enable_captions", True)
"""

if "merged_settings" in app_content and "caption_config" not in app_content:
  app_content = app_content.replace(
      'merged_settings["caption_config"] =', patch_code
  )
  with open("app.py", "w", encoding="utf-8") as f:
    f.write(app_content)
  print("✅ UI Settings mapped inside app.py successfully!")

print(
    "\n🎉 High-Capacity Setup Complete! You can now render 150 to 350+ clips"
    " smoothly."
)