...
shutil.copy(pipeline_file, "master_pipeline.py.bak_bridge")
...
def process_multi_clip_render(clips_list, audio_path, output_path, settings=None):
    ...
    # Bridge: Harmonize UI dictionary key names
    bg_music_path = (
        settings.get("bg_music_path") or 
        settings.get("bg_music") or 
        settings.get("music") or 
        settings.get("bgm_path")
    )
    sfx_files = (
        settings.get("sfx_files") or 
        settings.get("sfx_list") or 
        settings.get("sfx") or 
        settings.get("sound_effects") or []
    )
    logo_path = (
        settings.get("logo_path") or 
        settings.get("logo") or 
        settings.get("watermark") or 
        settings.get("logo_file")
    )
    cta_path = (
        settings.get("cta_path") or 
        settings.get("cta") or 
        settings.get("subscribe_overlay") or 
        settings.get("cta_video")
    )
    bg_volume = float(settings.get("bg_volume") or settings.get("music_volume") or 0.15)
    ...
    if bg_music_path and os.path.exists(str(bg_music_path)):
        ...
        print("   [INFO]: No Background Music provided or path invalid.")
    ...
    if logo_path and os.path.exists(str(logo_path)):
        ...
        print("   [INFO]: No Logo/Watermark file provided.")

    if cta_path and os.path.exists(str(cta_path)):
        try:
            cta_start = target_duration * 0.2
            cta_raw = VideoFileClip(str(cta_path)).resize(width=420)
            ...
            print(f"   [APPLIED IN FINAL]: Animated Subscribe CTA ({os.path.basename(str(cta_path))}) at t={cta_start:.2f}s.")
        except Exception as cta_err:
            print(f"   [WARN] Subscribe CTA Overlay Error: {cta_err}")
    else:
        print("   [INFO]: No CTA Video file provided.")

    master_composition = CompositeVideoClip(overlay_clips, size=final_video.size)

    # 6. EXPORT & CAPTION ENGINE
    ...
    caption_cfg = settings.get("caption_config") or settings.get("captions") or {"enabled": True, "style_id": "clean_subtitle", "mode": "phrase"}
    if isinstance(caption_cfg, dict) and caption_cfg.get("enabled", True) and os.path.exists(temp_export):
        try:
...
try:
  ...
  print(
      "🚀 SUCCESS: master_pipeline.py updated with dynamic key mapping bridge!"
  )
