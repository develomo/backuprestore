"""
============================================================================
CLEAN ALL-IN-ONE v2 — Phases 2+3+4+5 from Phase 1 Backup
============================================================================
INSTRUCTIONS:
  ...
  2. RUN:
     python clean_all_v2.py

This script applies Phase 2+3+4+5 on the clean Phase 1 backup,
with syntax validation at every step.
============================================================================
"""
import os
import re
...
def main():
    ...
    # ================================================================
    # PHASE 2: Unified Settings (preset_selector + settings_section)
    # ================================================================
    print("\n--- Phase 2: Unified Settings ---")

    # 2a: Replace preset_selector_section() — add auto-detect inside
...
'''
        ...
        print("  [SKIP 2a] preset_selector_section not found")

    # 2b: Replace settings_section() — unified all settings
...
'''
            ...
            fixes += 1
            print("  [2b] settings_section() — unified all settings")
            if not validate(content, "After 2b"):
                print("[STOP]")
                return
    else:
        print("  [SKIP 2b] settings_section not found")

    # 2c: Update _page_generator() — unified flow
    old_pg = '''    # 1. Preset Selector
    ...
    new_pg = '''    # 1. Preset Selector (niche + auto-detect + preset buttons)
    preset_num, preset_label = preset_selector_section()

    st.session_state["editing_preset_number"] = preset_num
    st.session_state["editing_preset_label"] = preset_label

    # 2. Settings (ALL + custom overrides in ONE expander)
    import json
    settings = settings_section()
    st.session_state["phase4_config"] = json.dumps({
        "preset_number": preset_num,
    ...
    if old_pg in content:
        ...
        fixes += 1
        print("  [2c] _page_generator() — unified flow, removed auto_detect as separate section")
        if not validate(content, "After 2c"):
            print("[STOP]")
            return
    else:
        print("  [SKIP 2c] _page_generator block not matching")
    ...
    old_cv = """    # 9. Output
    output_preview()"""
    # Check if caption_video_preview_section() exists before "output_preview()"
    if 'caption_video_preview_section()' in content:
        cps_idx = content.find('caption_video_preview_section()')
        # Find the line before it (likely a st.markdown("---") and comment)
        before_cps = content.rfind('\n', 0, cps_idx)
        before_before = content.rfind('\n', 0, before_cps)
        # Remove from the comment line before caption to the output_preview line
        # Find output_preview() after cps
        op_idx = content.find('output_preview()', cps_idx)
        if op_idx > 0:
            remove_start = content.rfind('#', 0, cps_idx)
            if remove_start < 0:
                remove_start = content.rfind('st.markdown("---")', 0, cps_idx)
            if remove_start > 0:
                remove_block = content[remove_start:op_idx]
                content = content.replace(remove_block, '')
                fixes += 1
                print("  [3a] Removed caption_video_preview_section() block")
    ...
    if old_warn in content:
        ...
        fixes += 1
        print("  [3b] Fixed caption preview fallback (clean HTML)")
        if not validate(content, "After 3b"):
            print("[STOP]")
            return
    else:
        print("  [SKIP 3b] warning not found (may already be fixed)")
    ...
    if old_pl in content:
        ...
        if not validate(content, "After 3c"):
            print("[STOP] Rolling back 3c...")
...
'''
        ...
        print("  [SKIP 5a] css() not found")

    # 5b: Theme toggle JS
    for old in ['window.toggleTheme();', 'try{window.toggleTheme();}catch(e){}']:
        if old in content:
            new_js = old.replace('toggleTheme', 'toggleThemeV2')
            content = content.replace(old, new_js)
            fixes += 1
            print(f"  [5b] Theme toggle JS updated")
    ...
    if validate(content, "FINAL STATE"):
        ...
        print("=" * 60)
        print(f"\n[SUCCESS] {fixes} fixes applied")
        print(f"[INFO] {init_lines} → {new_lines} lines")
        print("\n▶ streamlit run app.py")
    else:
        print("=" * 60)
        print("\n[FAILED] app.py NOT modified. Fix and retry.")
