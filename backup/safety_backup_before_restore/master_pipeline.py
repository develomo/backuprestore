...
try:
    ...
    # MoviePy not available - FFmpeg will be used instead
...
def generate_subtitle_segments(audio_path, caption_type="phrase"):
    ...
    # FFmpeg Fix: duration = float(subprocess.check_output(['ffprobe', '-v', 'error', '-show_entries', 'format=duration', '-of', 'default=noprint_wrappers=1:nokey=1', audio_path]).decode('utf-8').strip())
...
def process_multi_clip_render(clips_list, audio_path, output_path, settings=None):
    # --- FAIL-SAFE LONG VIDEO AUTOMATIC ROUTER ---
    try:
    ...
    try:
        ...
        final_video.close()
        # voice_audio.close() # Removed (FFmpeg)
